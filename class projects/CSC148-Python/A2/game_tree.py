...
def leaf_count(game_state_node):
    ...
    return (len(game_state_node.children) == 0 and 1) or \
           sum([leaf_count(c) for c in game_state_node.children])
...
def distinct_node_count(game_state_node):
    ...
    def _mark_distinct_nodes(game_state_node, node_states):
        '''Record distinct nodes in game_state_node into node_states.'''
        sig = str(game_state_node.value)
        if sig not in node_states:
            node_states[sig] = 1
            children = game_state_node.children
            if children:
                for c in children:
                    _mark_distinct_nodes(c, node_states)
    node_states = {}
    _mark_distinct_nodes(game_state_node, node_states)
    return len(node_states)
